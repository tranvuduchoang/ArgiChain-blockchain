const { ethers } = require('hardhat');

async function main() {
  console.log('🧪 Testing buyProduct directly...');
  
  const buyerAddress = '0xDE800c6278D82813AFd5104c365693C19B82F206';
  const marketplaceAddress = '0xF081DD1D2b30937c3c4c75728bBaA284407dDFCF';
  const tokenAddress = '0x76f84A7626914a3c762ff36518AdAFE16E30171b';
  
  // Get contracts
  const marketplace = await ethers.getContractAt('AgriChainMarketplace', marketplaceAddress);
  const token = await ethers.getContractAt('AgriChainToken', tokenAddress);
  
  // Get buyer account (we need to use the actual buyer account)
  const [deployer] = await ethers.getSigners();
  console.log('Deployer address:', deployer.address);
  
  // Check if we can call buyProduct as deployer
  try {
    console.log('📋 Getting listing details...');
    const listing = await marketplace.listings(1);
    console.log('Price:', ethers.formatEther(listing[3]), 'AGRI');
    
    const quantity = 1;
    const totalPrice = listing[3] * BigInt(quantity);
    console.log('Total price:', ethers.formatEther(totalPrice), 'AGRI');
    
    // Check deployer's token balance
    const deployerBalance = await token.balanceOf(deployer.address);
    console.log('Deployer token balance:', ethers.formatEther(deployerBalance), 'AGRI');
    
    if (deployerBalance < totalPrice) {
      console.log('❌ Deployer does not have enough tokens');
      return;
    }
    
    // Approve tokens
    console.log('✅ Approving tokens...');
    const approveTx = await token.approve(marketplaceAddress, totalPrice);
    await approveTx.wait();
    console.log('✅ Tokens approved');
    
    // Try to buy product
    console.log('🛒 Buying product...');
    const buyTx = await marketplace.buyProduct(1, quantity);
    console.log('📝 Buy transaction sent:', buyTx.hash);
    
    const receipt = await buyTx.wait();
    console.log('✅ Buy transaction confirmed:', receipt.hash);
    
    // Check if NFT was transferred
    const nftAddress = '0x0124b7D07Ebd0E9EDaACCdD126375671c3506f70';
    const nft = await ethers.getContractAt('AgriChainNFT', nftAddress);
    const nftBalance = await nft.balanceOf(deployer.address, 1);
    console.log('Deployer NFT balance after purchase:', nftBalance.toString());
    
  } catch (error) {
    console.error('❌ Error during buyProduct:', error.message);
    console.error('Full error:', error);
  }
}

main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error('❌ Error:', error);
    process.exit(1);
  });
