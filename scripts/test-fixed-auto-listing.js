...
async function main() {
  console.log("🧪 Testing Fixed Auto Listing Flow...");
  ...
  const mintTx = await nftContract.mintProductNFT(
    1, // amount
    "Test Fixed Auto Listing Product", // name
    "Product for testing fixed auto listing", // description
    "VEGETABLES", // category
    ethers.parseEther("0.003"), // price (0.003 AGRI)
    1, // quantity
    "kg", // unit
    true, // isOrganic
    Math.floor(Date.now() / 1000), // harvestDate
    "Test Farm", // location
    "https://agrichain.com/metadata/fixed-auto-test" // metadata
  );
  ...
  if (nftBalance == 0) {
    console.log("❌ NFT was not minted correctly");
    return;
  }

  console.log("\n=== STEP 3: SIMULATE AUTO LISTING (FIXED) ===");
  ...
  // Verify NFT balance before listing
  console.log("🔍 Verifying NFT balance before listing...");
  const balanceBeforeListing = await nftContract.balanceOf(deployer.address, newTokenId);
  console.log("NFT balance before listing:", balanceBeforeListing.toString());

  if (balanceBeforeListing == 0) {
    console.log("❌ No NFT balance to list");
    return;
  }

  // Create listing
  const price = ethers.parseEther("0.003"); // 0.003 AGRI
  const quantity = 1;
  ...
  console.log("\n🎉 Fixed Auto Listing Flow Test Completed Successfully!");
  console.log("✅ NFT minted with token ID:", newTokenId.toString());
  console.log("✅ Listing created with ID:", listingId.toString());
  console.log("✅ No 'Insufficient NFT balance' error occurred");
